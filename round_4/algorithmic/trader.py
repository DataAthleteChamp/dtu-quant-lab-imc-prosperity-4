...
# ── Counterparty Flow Signal (R4 — "Hello, I'm Mark") ─────────────────────
# Round-4 capsule (3 days, ~4280 trades) avg-forward-$ at 200-tick horizon
# splits Marks into informed vs noise:
#   Mark 14: HP +$7.96, VEV_4000 +$10.28  →  follow
#   Mark 38: HP -$7.74, VEV_4000 -$10.26  →  fade (mirror M14, top trade pair)
#   Mark 01: VFX +$2.24, VEV_5200 +$5.75  →  mild buy-side info
#   Mark 22: VEVs/HP slightly negative   →  uninformed seller (M01 counterparty)
#   Mark 49/55: VFX sellers, both lose   →  fade
#   Mark 67: VFX buyer +$0.90            →  mild long
# Pairs M14↔M38 (HP, VEV_4000) and M01↔M22 (VEVs) account for ~70% of volume.
# We hold per-(product, mark) decayed signed-flow and convert to FV bias via
# per-Mark β coeffs. β magnitudes set conservatively (≈10–25% of empirical
# fwd return) so the live BT-divergence factor (~0.5–0.8 on aggressive legs,
# per Round-3 audit) doesn't blow up live PnL.

class CounterpartyTracker:
    HALF_LIFE_TICKS = 50
    DECAY_PER_TICK = 0.5 ** (1.0 / 50.0)  # ≈0.9862

    # β calibration (R4 capsule, full-limit BT vs round3 baseline +97,185):
    #   HP β=0.20 lifts +25,174 across 3d (+8,984/-440/+16,630). Strong winner.
    #   VFX β=0.07 lifts D3 +5,592 but hurts D1/D2 by -9,350. Halved here so
    #   D1/D2 mean-reverting regime is less perturbed; the mid-EMA anchor
    #   already captures most of VFX's slow drift.
    # Voucher cp bias disabled: BT marginal (+72 on VEV_4000 D3 but -336 on
    #   VEV_5300 D2 and -128 on VEV_4000 D1). Smile-fit BS fair already does
    #   most of the work; directional bias chases in mean-reverting books.
    # HP β=0.20: M14 informed (avg fwd +$7.96/contract at 200t horizon),
    # M38 anti-informed (-$7.74). Verified consistent across all 3 capsule
    # days. Capsule BT lift +25,174 vs round-3 baseline.
    BETA: Dict[str, Dict[str, float]] = {
        "HYDROGEL_PACK":       {"Mark 14": 0.20, "Mark 38": -0.20},
    }
    BIAS_CLAMP: Dict[str, float] = {
        "HYDROGEL_PACK": 12.0,
    ...
    def __init__(self) -> None:
        ...
        self.last_seen_ts: int = -1

    def update(self, state: TradingState) -> None:
        for k in list(self.flow.keys()):
            self.flow[k] *= self.DECAY_PER_TICK
            if abs(self.flow[k]) < 0.05:
                del self.flow[k]
        max_ts = self.last_seen_ts
        for product, trades in (state.market_trades or {}).items():
            for t in trades or []:
                if t.timestamp is None or t.timestamp <= self.last_seen_ts:
                    continue
                if t.timestamp > max_ts:
                    max_ts = t.timestamp
                self._ingest(product, t.buyer, t.seller, t.quantity)
    ...
    def _ingest(self, product: str, buyer: Optional[str], seller: Optional[str], qty: int) -> None:
        if buyer and isinstance(buyer, str) and buyer.startswith("Mark"):
            self.flow[(product, buyer)] = self.flow.get((product, buyer), 0.0) + float(qty)
        if seller and isinstance(seller, str) and seller.startswith("Mark"):
            self.flow[(product, seller)] = self.flow.get((product, seller), 0.0) - float(qty)

    def bias(self, product: str) -> float:
        coeffs = self.BETA.get(product, {})
        if not coeffs:
            return 0.0
        b = 0.0
        for mark, beta in coeffs.items():
            b += beta * self.flow.get((product, mark), 0.0)
        cap = self.BIAS_CLAMP.get("VEV" if product.startswith("VEV_") else product, 3.0)
        if b > cap:
            b = cap
        elif b < -cap:
            b = -cap
    ...
    def to_dict(self) -> dict:
        return {
            "f": {f"{p}|{m}": round(v, 3) for (p, m), v in self.flow.items()},
            "t": self.last_seen_ts,
        }

    @classmethod
    def from_dict(cls, d: Optional[dict]) -> "CounterpartyTracker":
        obj = cls()
        if not d:
            return obj
        for k, v in (d.get("f") or {}).items():
            if "|" in k:
                p, m = k.split("|", 1)
                obj.flow[(p, m)] = float(v)
        try:
            obj.last_seen_ts = int(d.get("t", -1))
...
PARAMS_VEV_SPOT = {
    ...
    # R4 live: VFX started at 5,295.5 (above old 5,290 ceiling) and ranged
    # 5,247–5,300. Old clamp pinned FV at 5,290; trader sold every bid above
    # 5,293 → accumulated +172 long via late-day mean-reversion fills, but
    # the upper clamp also held back early-day correct-direction trades.
    # Widened to ±60 around 5250 = [5190, 5310] to cover live range.
    "anchor_clamp": (5190.0, 5310.0),
    "take_thresh": 3.0,
    "soft_cap_frac": 0.30,          # MM keeps |pos| ≤ 60 of 200 (light MM)
    "post_dist": 2,
    "make_disabled": True,
    # R4: VFX MR overlay disabled (was R3 v04e). On R4 capsule it cost
    # ~3.9k across 3 days, mostly D3 where it chased the downward drift.
    # The base TAKE+anchor captures the steady-state MR edge already.
    "mr_enabled": False,
    "mr_ema_window": 20,
    "mr_k_thr": 5.0,
    "mr_max_size": 20,
...
PARAMS_VOUCHER = {
    "tte_days_live": 4,             # R4 live = 4d (was 5 for R3). Capsule BT
                                    # handles via day_count rollover. auto_iv
                                    # back-solves σ each tick, partly hides
                                    # TTE error, but this aligns m=log(K/S)/√T
                                    # for the smile fit on the LIVE day.
    "sigma_global": 0.256,
...
PARAMS_VOUCHER_V05 = {
    ...
    # R4: VEV_5300 added to skip_quote — keeps it in the smile fit (anchor)
    # but no passive quotes are posted for it. Net capsule passive-PnL is
    # near zero (-1.2k/-1.2k/+1.3k across D1/D2/D3) but the volatility risk
    # in live is real (D3 had a -9k drawdown peak before recovering). The
    # MR overlay is also disabled (PARAMS_VOUCHER_MR), so the strike now
    # contributes only to smile fitting, no trades.
    "skip_quote_strikes": (5400, 5200, 5300),
    "wide_edge_strikes": {},
}
...
PARAMS_VOUCHER_MR = {
    # R4 defensive: all MR overlays disabled. The R3 outlier-detector tuning
    # was based on R3 live data and didn't generalize to R4 live (-5k live
    # vs +122k BT). Capsule BT cost ~2k from disabling these but live should
    # be cleaner.
    5300: {"enabled": False, "W": 40, "k_thr": 3.0, "max_size": 30},
    5400: {"enabled": False, "W": 20, "k_thr": 1.0, "max_size": 10},
    5500: {"enabled": False, "W": 40, "k_thr": 1.0, "max_size": 30},
}
...
def _trade_hp_as_mm(state: TradingState, saved: dict) -> List[Order]:
    ...
    # Rolling history for σ (std-dev of consecutive mid diffs) — on raw_mid.
    ...
    # Slow EMA reference for the dev-skip filter — on raw_mid.
    ema = saved.get("hp_ema")
    if ema is None:
        ema = raw_mid
    ema = (1 - p["ema_alpha"]) * ema + p["ema_alpha"] * raw_mid
    saved["hp_ema"] = ema

    # Skip MM under suspected informed flow (vs slow EMA, on raw mid).
    if abs(raw_mid - ema) > p["dev_skip_k"] * max(sigma, 1e-6):
        return []

    # Counterparty bias shifts the reservation-price center (Mark 14 informed
    # → mid up, Mark 38 anti-informed → mid down). σ/EMA stay on raw_mid.
    mid = raw_mid + float(saved.get("cp_bias_HP", 0.0))
...
def _trade_hp_static(state: TradingState, saved: dict) -> List[Order]:
    ...
    try:
        ...
        # R4 hysteretic FV anchor:
        #  - When rolling-200 median is within ±15 of canonical 9990 (HP is
        #    pinned), trust canon → preserves the capsule MM edge from the
        #    fixed-FV strategy (v23 BT +128k relies on this).
        #  - When median is more than ±15 away (live drift regime, e.g. live
        #    day mean 10,020), follow the median (clamped to ±50). This
        #    fixes the live -2,589 HP bleed where the trader was stuck at
        #    a 10,005 ceiling while market mean was 10,020 (selling every
        #    tick → max-short -200).
        #  - Warmup (len < 50): use latest mid clamped ±50 around canon, so
        #    we don't start selling at 10,008 against a 9990 FV.
        # R4: tight ±15 clamp around canon=9990 (v23 capsule winner). Live
        # day showed mid mean 10,020 — outside this clamp — so the trader
        # systematically sold short to -200 for a -2,589 HP bleed. Two
        # orthogonal fixes for live without breaking capsule:
        #  (a) the position-cap reduction below (max_pos 200→80) caps the
        #      blast radius if FV mismatch persists.
        #  (b) widening the clamp itself was tested and broke the capsule
        #      pinned-regime edge (drops to -68k on D2) because the
        #      adaptive median follows brief dips and creates wrong-side
        #      inventory. Kept tight.
        canon = float(rp["static_fv_anchor"])
        ahist = saved.get("hp_anchor_mids", [])
        if ahist and len(ahist) >= 50:
            ss = sorted(ahist)
            nn = len(ss)
            med = ss[nn // 2] if nn % 2 == 1 else 0.5 * (ss[nn // 2 - 1] + ss[nn // 2])
            anchor = max(canon - 15.0, min(canon + 15.0, med))
        else:
            anchor = canon
        fv = anchor + float(saved.get("cp_bias_HP", 0.0))

        # R4 regime-mismatch skip: if the rolling-200 median has been far
        # from canon for a SUSTAINED period (50+ consecutive ticks), the
        # static MM-around-9990 strategy systematically takes wrong-side
        # inventory (live day mean 10,020 caused -2,589 HP bleed). The
        # consecutive-tick counter filters out transient capsule dips
        # (mean-reverting D2 saw the median briefly leave the band but
        # always returned within ~20 ticks).
        if ahist and len(ahist) >= 50:
            if abs(med - canon) > 12.0:
                saved["hp_drift_streak"] = saved.get("hp_drift_streak", 0) + 1
            else:
                saved["hp_drift_streak"] = 0
            if saved.get("hp_drift_streak", 0) > 50:
                return []
...
def _trade_hp_static_trend(state: TradingState, saved: dict) -> List[Order]:
    ...
    try:
        ...
        # R4 cp bias on rolling anchor (M14 buy → anchor up, M38 buy → down)
        anchor += float(saved.get("cp_bias_HP", 0.0))
...
def trade_hydrogel(state: TradingState, saved: dict) -> List[Order]:
    ...
    if regime == "trending":
        # R4 — with cp signal, fall through to _trade_hp_static (adaptive
        # rolling-200 median anchor + cp bias). Without cp signal: freeze.
        cp_strong = abs(float(saved.get("cp_bias_HP", 0.0))) >= 1.5
        if cp_strong:
            return _trade_hp_static(state, saved)
...
def trade_vev_spot(state: TradingState, saved: dict, hedge_size: int) -> List[Order]:
    ...
    # R4: counterparty bias (M01/M67 long-info, M49/M55/M22 fade) — clamped ±4
    fair += float(saved.get("cp_bias_VFX", 0.0))
...
def trade_voucher_v05a(state: TradingState, saved: dict, K: int, S: float,
    ...
    # R4 counterparty bias on voucher fair (Mark 14/38 dominate VEV_4000;
    # Mark 01/22 dominate near-the-money strikes 5200-5500). Bias is in
    # voucher price units, clamped ±3 by CounterpartyTracker.bias().
    cp_bias_vev_map = saved.get("cp_bias_VEV") or {}
    cp_bias_K = float(cp_bias_vev_map.get(str(K), 0.0))
    fair += cp_bias_K
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # ── R4 Counterparty signal — update tracker, expose biases for handlers
        try:
            cp = CounterpartyTracker.from_dict(saved.get("cp"))
            cp.update(state)
            saved["cp"] = cp.to_dict()
            saved["cp_bias_HP"] = cp.bias("HYDROGEL_PACK")
            saved["cp_bias_VFX"] = cp.bias("VELVETFRUIT_EXTRACT")
            saved["cp_bias_VEV"] = {
                str(K): cp.bias(f"VEV_{K}") for K in (4000, 5200, 5300, 5400, 5500)
            }
        except Exception as e:
            logger.print(f"CP_TRACKER ERR: {e}")
            saved.setdefault("cp_bias_HP", 0.0)
            saved.setdefault("cp_bias_VFX", 0.0)
            saved.setdefault("cp_bias_VEV", {})
